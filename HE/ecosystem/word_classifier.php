...
$q1 = "SELECT id,txt,t FROM " . $prefix . "content WHERE processed_classification=0 LIMIT 0,300";
$r1 = $dbh->query($q1);
if($r1){
	foreach ( $r1 as $row1) {
		...
		for($i = 0; $i<count($parts1); $i++){
			...
			if(strlen($w1)>$minL){
				...
				$id1 = -1;
				$n1 = -1;
				$q2 = "SELECT id,n FROM " . $prefix . "classifier_words WHERE word='" . $w1 . "' LIMIT 0,1";
				$r2 = $dbh->query($q2);
				if($r2){
					foreach ( $r2 as $row2) {
						// yes --> add 1 to n
				...
				if($id1==-1 && $n1==-1){
					$n1 = 1;
					$q3 = "INSERT INTO classifier_words(word,n) VALUES('" . $w1 . "'," . $n1 . ")";
					$r3 = $dbh->query($q3);
		...
		for($i = 0 ; $i<count($idwords); $i++){
			...
			for($j = $i+1 ; $j<count($idwords); $j++){
				$id2 = $idwords[$j];
				$q3 = "SELECT id,n FROM classifier_corecurrence WHERE (idw1=" . $id1 . " AND idw2=" . $id2 . ") OR (idw1=" . $id2 . " AND idw2=" . $id1 . ") LIMIT 0,1";
				$r3 = $dbh->query($q3);
				$idcorr = -1;
				$ncorr = -1;
				if($r3){
				...
				if($idcorr==-1 && $ncorr==-1){
					$ncorr = 1;
					$q4 = "INSERT INTO classifier_corecurrence(idw1,idw2,n) VALUES(" . $id1 . "," . $id2 . "," . $ncorr . ")";
					$r4 = $dbh->query($q4);
				...
				$idcome = -1;
				$ncome = -1;
				$q5 = "SELECT id,n FROM classifier_corecmessage WHERE idcorr=" . $idcorr . " AND idcontent=" . $idco . " LIMIT 0,1";
				$r5 = $dbh->query($q5);
				if($r5){
					foreach ($r5 as $row5) {
						$idcome = $row5["id"];
				...
				if($idcome==-1 && $ncome==-1){
					$ncome = 1;
					$q6 = "INSERT INTO classifier_corecmessage(idcorr,idcontent,t,n) VALUES(" . $idcorr . "," . $idco . ",'" . $t1 . "'," . $ncome . ")";
					$r6 = $dbh->query($q6);
				}
			}			
		}
